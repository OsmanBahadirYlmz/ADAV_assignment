...
ggplot(correlation_df, aes(Var2, Var1, fill = value)) +
  ...
  scale_fill_gradient2(low = "blue", high = "red", mid = "white", 
                       ...
                       name="Correlation") +
  theme_minimal() +
  labs(title = "Correlation HeatMap", x = "Features 1", y = "Features 2")+
...
```
The above heatmap illustrates the correlation coefficients between pairs of features in the pokemon dataset. Each tile in the heatmap represents the correlation between two features.
...
vif_values <- vif(lm(base_total ~ ., data = df))
vif_df <- data.frame(Parameter = names(vif_values), VIF = vif_values)

# Print VIF values
print(vif_values)

# Visualize VIF values
ggplot(vif_df, aes(x = reorder(Parameter, VIF), y = VIF)) +
  geom_bar(stat = "identity", fill = "dodgerblue") +
  coord_flip() +
  labs(title = "Variance Inflation Factor (VIF) for Predictors", x = "Parameter", y = "VIF") +
  theme_minimal()+
  theme(axis.text.y = element_text(size = 7))


```
The graph shows the Variance Inflation Factor (VIF) values for each predictor in the pokemon dataset. Each bar represents a predictor, and the height of the bar indicates the VIF value.
...
train_index <- createDataPartition(df$base_total, p = .7, 
                                  list = FALSE, 
                                  times = 1)
